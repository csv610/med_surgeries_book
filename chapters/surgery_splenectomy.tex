...
A splenectomy is the surgical removal of the spleen, an essential lymphoid organ located in the left upper quadrant of the abdomen, beneath the diaphragm and posterior to the stomach. This procedure is indicated in various clinical scenarios, including traumatic splenic rupture, hematologic disorders such as immune thrombocytopenia (ITP) and hereditary spherocytosis, as well as certain malignancies involving the spleen. The spleen plays a critical role in the immune response, filtering blood, recycling iron from hemoglobin, and storing platelets. Its removal can lead to significant alterations in a patient's hematologic profile and immune function, necessitating careful postoperative management to mitigate risks, particularly overwhelming post-splenectomy infection (OPSI). Splenectomy can be performed using either an open or a minimally invasive (laparoscopic) approach, with the choice of technique depending on the clinical context, the size of the spleen, and the surgeon's expertise.
...
\begin{itemize}
  ...
  \item \textbf{Vaccinations:} For elective splenectomy, vaccination should be planned ideally at least 14 days before surgery when feasible, using current age- and risk-based pneumococcal, meningococcal, and Hib recommendations rather than a fixed PCV13/PPSV23 schedule. If urgent splenectomy is required, vaccination is given after recovery according to current guidance.
...
\textbf{Situations requiring optimization or an alternative plan:}
\begin{itemize}
  ...
  \item Diffuse metastatic cancer or severe comorbidity may make elective splenectomy inappropriate unless there is a clear therapeutic or palliative benefit.
\end{itemize}
...
Outcomes depend on the indication, urgency, associated injuries, age, immune status, and surgical approach. Splenectomy may improve selected hematologic disorders and provide hemorrhage control in trauma, but it does not guarantee remission or survival. Lifelong asplenia precautions include vaccination, prompt assessment of fever, and an individualized plan for antibiotic prophylaxis and travel-related infection risk.
